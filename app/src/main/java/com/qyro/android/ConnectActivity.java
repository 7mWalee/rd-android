...
public class ConnectActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LinearLayout.LayoutParams hostParams = new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        hostParams.bottomMargin = 16;
        hostField.setLayoutParams(hostParams);
        layout.addView(hostField);

        EditText tokenField = new EditText(this);
        tokenField.setHint("Auth Token (paste here)");
        tokenField.setTextColor(0xFFFFFFFF);
        tokenField.setHintTextColor(0xFF888888);
        tokenField.setBackgroundColor(0xFF1A1A22);
        tokenField.setPadding(20, 20, 20, 20);
        LinearLayout.LayoutParams tokenParams = new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        tokenParams.bottomMargin = 32;
        tokenField.setLayoutParams(tokenParams);
        ...
        connectBtn.setOnClickListener(v -> {
            ...
            if (!token.isEmpty()) {
                AuthStore.saveToken(host + ":" + port, user, token);
            }
